...
public class ZSMyService: NSObject {
...
//            let books = [ZSUserBookshelf].deserialize(from: json?["books"] as? [Any]) as? [String:Any]
    ...
    public func fetchNicknameChange(url:String, param:[String:Any]?, completion:@escaping ZSBaseCallback<[String:Any]>) {
        zs_post(url, parameters: param) { (json) in
            completion(json)
        }
    }
    
    public func fetchVoucherList(url:String, param:[String:Any]?, completion:@escaping ZSBaseCallback<[ZSVoucher]>) {
