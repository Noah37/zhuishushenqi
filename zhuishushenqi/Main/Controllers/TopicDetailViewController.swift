...
class TopicDetailViewController: BaseViewController ,SegMenuDelegate,UITableViewDataSource,UITableViewDelegate{
    ...
    var id:String? = ""
    var books:NSArray? = []
    fileprivate let iden = "TopicDetailCell"
    ...
    fileprivate lazy var tableView:UITableView = {
        ...
        tableView.register(UINib (nibName: self.header, bundle: nil), forCellReuseIdentifier: self.header)
        return tableView
    }()
...
//        http://api.zhuishushenqi.com/book-list/58b782f5a7674a5f67618731
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            let headerCell:TopicDetailHeaderCell? = tableView.dequeueReusableCell(withIdentifier: header, for: indexPath) as?  TopicDetailHeaderCell
            headerCell?.backgroundColor = UIColor.white
            headerCell?.selectionStyle = .none
            headerCell?.model = headerModel
            return headerCell!
        }
        let cell:TopicDetailCell? = tableView.dequeueReusableCell(withIdentifier: iden,for: indexPath) as? TopicDetailCell
