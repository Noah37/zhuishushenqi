...
public class QSManager:NSObject{
    ...
    public static let defaultHTTPHeaders: HTTPHeaders = {
        ...
        let userAgent: String = {
            ...
            return "YouShaQi/4.4.4 (iPhone; iOS 12.0; Scale/3.00)"
        }()
        
        return [
            "Accept-Encoding": acceptEncoding,
            "Accept-Language": acceptLanguage,
            "User-Agent": "YouShaQi/4.4.4 (iPhone; iOS 12.0; Scale/3.00)"
        ]
    }()
...
//        let semaphore = DispatchSemaphore(value: 0)
        ...
        let task = session?.downloadTask(with: urlRequest) { (url, response, error) in
            let cachesPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending("/\(response?.suggestedFilename ?? "")") ?? ""
            let moveURL = URL(fileURLWithPath: cachesPath)
            let urlResponse:HTTPURLResponse? = response as? HTTPURLResponse
            if urlResponse?.statusCode == 200 {
                if let fileURL = url {
                    do{
                        try FileManager.default.moveItem(at: fileURL, to: moveURL)
